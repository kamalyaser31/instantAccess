...
def make_event(event_type, name, scan_code=None, time=0):
	return KeyboardEvent(
		event_type=event_type, scan_code=scan_code or dummy_keys[name][0][0], name=name, time=time
	)
...
class TestKeyboard(unittest.TestCase):
	...
	def do(self, manual_events, expected=None):
		...
		if expected is not None:
			to_names = lambda es: "+".join(
				("d" if e.event_type == KEY_DOWN else "u") + "_" + str(e.scan_code) for e in es
			)
	...
	def test_on_press_nonblocking(self):
		keyboard.on_press(
			lambda e: self.assertEqual(e.name, "a") and self.assertEqual(e.event_type, KEY_DOWN)
		)
		self.do(d_a + u_a)
	...
	def test_on_release(self):
		keyboard.on_release(
			lambda e: self.assertEqual(e.name, "a") and self.assertEqual(e.event_type, KEY_UP)
		)
		self.do(d_a + u_a)
	...
	def test_on_press_key_nonblocking(self):
		keyboard.on_press_key(
			"A", lambda e: self.assertEqual(e.name, "a") and self.assertEqual(e.event_type, KEY_DOWN)
		)
	...
	def test_on_release_key(self):
		keyboard.on_release_key(
			"a", lambda e: self.assertEqual(e.name, "a") and self.assertEqual(e.event_type, KEY_UP)
		)
		self.do(d_a + u_a)
	...
	def test_add_hotkey_single_step_suppress_args_allow(self):
		arg = object()
		keyboard.add_hotkey(
			"a", lambda a: self.assertIs(a, arg) or trigger() or True, args=(arg,), suppress=True
		)
		self.do(d_a, triggered_event + d_a)
	...
	def test_parse_hotkey_combinations_list_list(self):
		self.assertEqual(
			keyboard.parse_hotkey_combinations(keyboard.parse_hotkey_combinations("a, b")),
